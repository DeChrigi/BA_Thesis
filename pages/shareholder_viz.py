...
def load_graph(filename, min_weight_filter):
    ...
    for node, data in G.nodes(data=True):
        if node == central_node:  # Falls es der 'Cartel' Knoten ist
            x, y = pos[node]
            cartel_x.append(x)
            cartel_y.append(y)
    ...
    cartel_trace = go.Scatter(
        ...
        text=cartel_text,
        marker=dict(
            color='#6aa84f',  # Hier setzen wir die Farbe auf Rot
            size=20,       # Optional: Größer als die anderen Knoten für Hervorhebung
            line_width=2
        )
    )
